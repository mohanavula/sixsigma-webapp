    ...
    <div class="modalbackdrop fixed inset-0 bg-gray-900 bg-opacity-75 flex justify-center items-center text-gray-900" >
		<div class="modal- bg-white shadow-md overflow-x-auto flex flex-col justify-start p-6 rounded-sm w-1/2 text-sm" >
			<header class="modalheader text-lg font-semibold border-b border-gray-600 flex" id="modalTitle" >
				<span name="header">Create Exam</span>
			...
			<section v-else class="modalbody py-2">
                ...
                <div class="flex items-start my-2"> <!-- exam category -->
					<label for="regulation" class="w-1/5">Exam category</label>
                    <div class="relative w-4/5">
                        <select v-model="selected_exam_category" class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-800 py-1 pl-1 pr-8 rounded-sm leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                            <option v-for="ec in ['REGULAR', 'SUPPLEMENTARY', 'MIDTERM']" :key="ec" :value="ec">{{ ec }}</option>
                        </select>
                ...
                <div class="flex items-start my-2"> <!-- start and end dates -->
					<label for="academic_year" class="w-1/5">Dates</label>
                    <div class="flex w-4/5">
                        <v-date-picker mode='range' v-model='dates_range' color="teal" locale="en-GB" class="flex-grow" :input-props='{readonly: true, }'>
                            <input id="date" slot-scope="{ inputProps, inputEvents, isDragging }"
                                :class="[`appearance-none border border-gray-300 bg-gray-200 rounded-sm w-full py-1 px-2 leading-tight mr-1 ${ isDragging ? 'text-gray-400' : 'text-gray-800' }`]"
                                v-bind="inputProps" v-on="inputEvents" placeholder="Select start and end dates">
                        </v-date-picker>
                    </div>
				</div>
				...
				<div class="flex items-start my-2"> <!-- semesters -->
					...
					<div v-if="filtered_semesters.length > 0" class="w-1/5 mx-3 mt-auto">
						<button :disabled="addExamsBtnDisabled" class="bg-gradient-gray-light  border-gray-500 text-sm border text-gray-900 py-1 px-2 rounded-sm" :class="addExamsBtnDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gradient-gray-dark hover:border-teal-700'" type="button" @click="add_exams()">
							Add Exams
						</button>
					</div>
				...
				<!-- scheduled exams -->
				<div class="flex items-start my-2 h-48 overflow-y-auto">
					<div class="w-1/5">Exams</div>
					<table class="w-4/5">
						...
						<tr v-for="exam in scheduled_exams" :key="exam.id" class="border-b border-gray-500">
							<td>{{ exam.program_name }}</td>
							<td>{{ exam.regulation_name }}</td>
							<td>{{ exam.semester_name }}</td>
							<td>{{ format_date(exam.start_date) }}</td>
							<td>{{ format_date(exam.end_date) }}</td>
							<td>
								<svg viewBox="0 0 20 20" fill="currentColor" class="trash w-4 h-4 text-teal-500 cursor-pointer" @click="delete_exam(exam)"><path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>
							</td>
						</tr>
						<tfoot v-show="scheduled_exams.length == 0">
							<tr>
								<td colspan="5">No exams are scheduled</td>
							</tr>
						</tfoot>
					</table>
				</div>
				<div class="flex">
					<div class="w-1/5"></div>
					<div class="w-4/5">
						<span class="mx-2">Total exams: {{ scheduled_exams.length }}</span>
					</div>
				</div>
			...
			<footer class="modalfooter pt-2 my-2 border-t border-gray-600">
				<div class="flex">
					...
					<button :disabled="cretateBtnDisabled" class="bg-gradient-gray-light border-gray-500 text-sm border text-gray-900 py-1 px-2 ml-2 rounded-sm" :class="cretateBtnDisabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gradient-gray-dark hover:border-teal-700'" type="button" @click="create_exam()">
						Create
					</button>
				</div>
				<div v-show="isCreating" class="text-sm my-2">
					Creating... {{ feedback }}
				</div>
			</footer>
		</div>
...
export default {
    ...
    data() {
        return {
            ...
            selected_exam_category: 'REGULAR',
            scheduled_exams: [],
            dates_range: {
                start: this.get_date_after_days(),
                end: this.get_date_after_days(24) 
            }
        }
    ...
    computed: {
        ...
        addExamsBtnDisabled() {
			return this.semesters.length == 0 || this.filtered_semesters.reduce((unselected, fs) => { return unselected && !fs.selected}, true) || this.dates_range == null
		},

		cretateBtnDisabled() {
			return this.scheduled_exams.length == 0 || this.dates_range == null
		}
    ...
    methods: {
        hideModal() {
			EventDispatcher.fire("hide-create-exam-modal");
        },
        ...
        add_exams() {
			...
			this.filtered_semesters.forEach(fs => {
				if (fs.selected)
					if (!this.scheduled_exams.reduce((repeat, sc) => {
						return repeat || sc.semester_id == fs.id
					}, false))
						this.scheduled_exams.push({
                            'id': Math.round(Math.random() * 1000000),
                            'short_name': "",
							'name': `${this.academic_year_plus_one}-${this.selected_regulation.short_name}-${this.selected_regulation.program.short_name}-${fs.short_name}-${this.selected_exam_category}`,
							'academic_year': this.academic_year,
							'start_date': this.dates_range.start,
							'end_date': this.dates_range.end,
							'semester_id': fs.id,
							'exam_category': this.selected_exam_category,
							'semester_name': fs.name,
							'program_name': this.selected_regulation.program.short_name,
							'regulation_name': this.selected_regulation.short_name,
						})
			})
        },
        
        get_date_after_days(days = 0) {
            let d = new Date()
        ...
        create_exam() {
			...
			axios.post("/academics/exams", {
				exams: this.scheduled_exams
			})
			.then(() => {
				this.feedback = "sent"
				setTimeout(() => {
        ...
        delete_exam(exam) {
            this.scheduled_exams = this.scheduled_exams.filter(se => {
                return se.id != exam.id
			})
		},
    ...
    created() {
        if (this.fetchedRegulations) this.selected_regulation = this.regulations[0]
